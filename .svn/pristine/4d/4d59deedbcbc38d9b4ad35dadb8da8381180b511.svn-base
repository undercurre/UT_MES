      ...
      <el-card>
        <div class="panel-search__results-group-inner">
          <d2-panel-search-item
            ...
            :item="item"
            :hover-mode="true"
            @click.native="handleResultsGroupItemClick(item.path, item)"/>
        </div>
      </el-card>
    </div>
  </div>
  ...
  computed: {
    ...
    resultsList () {
      return (this.results.length === 0 && this.searchText === '') ? this.pool.map(e => ({
        value: this.$i18n.locale === 'zh-chs' ? e.fullTitle : e.en_fullTitle,
        ...e
      })) : this.results
    }
  },
  methods: {
    ...
    query (pool, queryString) {
      return new Fuse(pool, {
        ...
        keys: [
          this.$i18n.locale === 'zh-chs' ? 'fullTitle' : 'en_fullTitle',
          'path'
        ]
      })
        .search(queryString)
     ...
     */
    handleResultsGroupItemClick (path, item) {
      ...
      // 用户选择的是其它页面
      this.handleMenuSelect(path, item, this.frameOut, '')
    },
    /**
    ...
    handleSelect (e) {
      ...
      this.$nextTick(() => {
        // eslint-disable-next-line camelcase
        this.handleMenuSelect(path, e, this.frameOut, '')
      })
    },
    /**
     * @augments 关闭输入框的下拉菜单
