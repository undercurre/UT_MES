...
<table class="ListTable table table-striped" style="font-size: 8px">
    ...
    <tbody class="table-body">
      <tr v-for="(item, index) in RoutesData" :key="index">
        <th style="width:30%" :class="item.CURRENT_BOOL?'red-txt':''">{{ item.CURRENT_NAME }}</th>
        <th style="width:30%" :class="item.CURRENT_BOOL?'red-txt':''">{{ item.REPAIR_NAME }}</th>
        <th style="width:30%" :class="item.CURRENT_BOOL?'red-txt':''">{{ item.REWORK_NAME }}</th>
      </tr>
    </tbody>
  </table>
      ...
      this.$nextTick(() => {
        ...
        for (let i = 0; i < data.length; i++) {
          for (let j = 0; j < this.currentBox.length; j++) {
            ...
            if (data[i].CURRENT_OPERATION_ID === this.OperationId) {
              data[i].CURRENT_BOOL = true
            }
          }
