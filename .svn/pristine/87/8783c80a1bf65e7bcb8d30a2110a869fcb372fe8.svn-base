  ...
  <custom-container>
    ...
    <div class="table-container">
      ...
      >
        ...
        <vxe-table-column
          field="SOURCE"
          :title="$t('SfcsEquipHead.SOURCE')"
          :edit-render="{ name: '$input', props: { readonly: true } }"
        />
        <vxe-table-column
          field="DEFECT_DESCRIPTION"
          :title="$t('SfcsEquipHead.DEFECT_DESCRIPTION')"
          :edit-render="{ name: '$input', props: { readonly: true } }"
        />
        <vxe-table-column
          field="LOCATION"
          :title="$t('SfcsEquipHead.LOCATION')"
          :edit-render="{ name: '$input', props: { readonly: true } }"
        />
        <vxe-table-column
          field="QTY"
          :title="$t('SfcsEquipHead.QTY')"
          :edit-render="{ name: '$input', props: { readonly: true } }"
        />
      </vxe-table>
    </div>
...
import { GetSfscEquipHeadCodeDataAsync } from '@/api/SfcsEquipHead'
export default {
  name: 'ScraperUsed',
  components: {
  ...
  methods: {
    async getLoadData () {
      this.loading = true
      const res = await GetSfscEquipHeadCodeDataAsync(this.formData)
