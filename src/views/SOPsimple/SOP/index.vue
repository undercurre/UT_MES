      ...
      <el-row class="row header-title sop-header">
        ...
        <el-col :span="4"
          ...
          <div class="kanban-title-tab"
               v-show="butoShow" v-if="!isEsopDrawer">
          ...
          <div class="kanban-title-tab" v-show="!butoShow">
            <span @click="autoInterMethod"  style="user-select: none;width:100px">{{isAutoInterval?'开启轮播':'关闭轮播'}}</span>
          </div>
        </el-col>
      </el-row>
      <!--工单信息栏-->
            ...
            <div style="padding: 0px 0; height: 100%; display: grid">
              <div class="pic-pn-div border-container"
                ...
                <el-col id="mapswp"
                  ...
                  <el-carousel class="SOPSimpleCarousel" trigger="click" :autoplay="!isAutoInterval"  :interval="5000">
                    <el-carousel-item :label="(index+1)" v-for="(item,index) in resourceData"
                                      :key="item.ID"
...
export default {
  ...
  methods: {
    // 是否自动轮播
    autoInterMethod () {
      this.isAutoInterval = !this.isAutoInterval
    },
    // 点击安灯按钮
    RolesClick () {
...
.SOPSimpleCarousel .el-carousel__button{
  background: #3e9ce2;
  color: #fff;
}
</style>
