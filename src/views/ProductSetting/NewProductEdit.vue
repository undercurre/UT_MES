<template>
  ...
  >
    ...
    <el-row ref="elRow1" style="display: flex; align-items: center">
      <el-col :span="22">
        <el-row>
           <!-- 项目编号 -->
        <el-col :span="4" class="compile-col" style="padding-right: 3px">
          <span class="sp-tit">项目编号:</span>
          <el-input
            v-model="mainTable.COLUMN_CODE"
            :disabled="mainTable.STATUS == 1"
            :placeholder="$t('SOPRoutes._200')"
            class="select"
          />
        </el-col>

        <el-col :span="4" class="compile-col">
        ...
        <el-col :span="5" class="compile-col" style="padding-right: 3px">
          <span class="sp-tit">物料名称:</span>
          <el-input
            v-model="mainTable.ROUTE_NAME"
            :placeholder="$t('SOPRoutes._58')"
            class="select"
          />
        </el-col>
        <el-col :span="11" class="compile-col">
          ...
          <el-button @click="SysfineClick" type="primary">{{
            $t("SfcsProductConfig._12")
          }}</el-button>
          <el-button
            @click="Edited"
            v-if="
              mainTable.STATUS == 0 && $btnList.indexOf('EditProcess') !== -1
            "
            type="primary"
            >{{ $t("SOPRoutes._192") }}</el-button
          >
          <el-button
            type="primary"
            v-if="
              mainTable.STATUS == 1 &&
              $btnList.indexOf('SfcsProductMultiSaveData') !== -1
            "
            @click="ConPlate"
            >{{ $t("spmr._30") }}</el-button
          >
          <el-button
            type="primary"
            v-if="
              mainTable.STATUS == 1 &&
              $btnList.indexOf('NewProductEditConfiguration') !== -1
            "
            @click="Configuration"
            >{{ $t("ProductSetting._1") }}</el-button
          >
        </el-col>
        </el-row>
        ...
        <el-row style="margin-top:30px">
          <!-- 分组标记 -->
        <el-col :span="4" class="compile-col" style="padding-right: 3px">
          <span class="sp-tit">{{ $t("slw.t_6") }}:</span>
          <el-input
            v-model="mainTable.PACKET_MARKING"
            disabled
            placeholder="请输入分组标记"
            class="select"
          />
        ...
        <!-- 项目名称 -->
        <el-col :span="5" class="compile-col" style="padding-right: 3px">
          <span class="sp-tit">项目名称:</span>
          <el-input
            v-model="mainTable.ATTRIBUTE3"
            disabled
            placeholder="请输入项目名称"
            class="select"
          />
        ...
        <!-- 检验方案 -->
        <el-row :span="11" class="compile-col">
           <span class="sp-tit">检验方案:</span>
        </el-row>
        </el-row>
    ...
    <!-- 表格 -->
    <el-row ref="elrow" :gutter="24" style="margin-top: 30px">
      <!-- 工序 -->
      <el-col :span="12" style="padding-right: 5px; height: 100%">
      ...
      >
        <el-tabs v-model="activeName" height="100%;" @tab-click="handleClick">
          <!-- 作业图  SOP维护-->
          <el-tab-pane
            :disabled="PrimaryData == null"
            :label="$t('ProductSetting._3')"
            name="operation"
          >
            <div style="margin-bottom: 15px">
              <el-input
                :placeholder="$t('ProductSetting._4')"
                :disabled="PrimaryData == null"
                style="width: 150px"
              ...
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('NewProductEditUploadResources') !== -1
                "
                @click="primary_upload_but()"
                >{{ $t("ProductSetting._5") }}</el-button
              >
              <!-- mainTable.STATUS == 0 &&  -->
              <el-button
                type="success"
                ...
                @click="primary_preview_but()"
                >{{ $t("SOPRoutes._72") }}</el-button
              >
              <!-- 文件系统 -->
              <el-button
                type="success"
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('SOPRoutestFileSystem') !== -1
                "
                @click="FileSystem()"
                >{{ $t("SfcsProductPallet._42") }}</el-button
              >
              <el-button
                type="success"
                :disabled="PrimaryData == null"
                ...
                "
                @click="workingMove"
                >{{ $t("SfcsProductPallet._49") }}</el-button
              >
              <el-button
                type="danger"
                :disabled="PrimaryData == null"
                v-if="
                  ...
                  $btnList.indexOf('SOPRoutestFileSystem') !== -1
                "
                @click="SelectCloumDelete"
                >{{ $t("SfcsProductPallet._48") }}</el-button
              >
            </div>
              ...
              >
                ...
                >
                  <template slot-scope="scope"
                    >{{ scope.row.RESOURCE_SIZE }} KB</template
                  >
                </el-table-column>
                <el-table-column
                ...
                >
                  <template slot-scope="scope">
                    ...
                    <el-button
                      ...
                      >{{ $t("SOPRoutes._79") }}</el-button
                    >
                    <el-button
                      type="success"
                      size="mini"
                      @click="opera_preview_but(scope.row)"
          ...
          <el-tab-pane
            :disabled="PrimaryData == null"
            v-if="$btnList.indexOf('CollectionPartsMaintenance') !== -1"
            :label="$t('ProductSetting._6')"
            name="Components"
          >
            <div style="margin-bottom: 15px">
              <el-select
                v-model="CollectPartsForm.COMPONENT_ID"
                clearable
                style="width: 150px"
                :placeholder="$t('ProductSetting._7')"
              >
                <el-option
                  v-for="item in ComponentName"
                  :key="item.ID"
              ...
              <el-input
                :placeholder="$t('ProductSetting._8')"
                style="width: 150px"
                clearable
                v-model="CollectPartsForm.ODM_COMPONENT_PN"
              />
              <el-button type="primary" @click="CollectPartsSearch">{{
              ...
              <el-button
                type="success"
                v-if="mainTable.STATUS == 0"
                @click.prevent="addCollectPart(0)"
              ...
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('collectionSfcsProductComponentsSave') !== -1
                "
                @click="SaveCollectPart()"
              ...
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('collectionSfcsProductComponentsdelete') !==
                    -1
                "
                @click="removeCollectPart()"
              ...
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('annexSaveData') !== -1
                "
                @click="annexClick()"
                >{{ $t("ProductSetting._50") }}</el-button
              >
            </div>
              ...
              >
                ...
                <!-- <vxe-table-column sortable show-overflow
                  ...
                  <template v-slot:edit="scope">
                    <vxe-select v-model="scope.row.COMPONENT_ID"
                                transfer>
                      <vxe-option v-for="item in ComponentNameBox"
                                  :key="item.value"
                                  :value="item.value"
                                  v-show="item.value"
                  ...
                  <template v-slot="{ row }">{{row.COMPONENT_ID}}</template>
                </vxe-table-column> -->
            ...
            <template v-slot="{ row }">{{ getSelectLabel(row.sex, sexList) }}</template>
                  ...
                  <template v-slot:edit="scope">
                    <div style="position: relative;">
                      <vxe-select v-model="scope.row.ODM_COMPONENT_PN"
                                  transfer>
                        <vxe-option v-for="item in CollectPartsDataSelect.map(item => item.COMPONENT_PN)"
                                    :key="item"
                                    :value="item"
                        ...
                        <div style="width: 600px;position: sticky;bottom: 6px;background: #fff;z-index: 90;padding-left: 15px">
                          <el-pagination :pager-count="5"
                                         ...
                                         :page-sizes="[15, 25, 35, 45]"
                                         layout="total, sizes, prev, pager, next, jumper"
                                         :total="CollectPartsDataSelect.length"
                ...
                >
                  <template v-slot:edit="scope">
                    <vxe-input v-model="scope.row.ODM_COMPONENT_PN"></vxe-input>
                  </template>
                  <template v-slot="{ row }">{{
                    row.ODM_COMPONENT_PN
                  }}</template>
                </vxe-table-column>
                ...
                >
                  <template v-slot:edit="scope">
                    <vxe-input v-model="scope.row.DATA_FORMAT"></vxe-input>
                  </template>
                  <template v-slot="{ row }">{{ row.DATA_FORMAT }}</template>
                </vxe-table-column>
                <!-- 数量 -->
                <vxe-table-column
                  sortable
          ...
          <el-tab-pane
            :disabled="PrimaryData == null"
            v-if="$btnList.indexOf('CollectionBoxNumberMaintenance') !== -1"
            :label="$t('ProductSetting._17')"
            name="CaseNumber"
          >
            <div style="margin-bottom: 15px">
              <el-input
                :placeholder="$t('ProductSetting._18')"
                style="width: 150px"
                clearable
              ...
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('CaseSfcsProductCartonSave') !== -1
                "
                @click="SaveCase()"
                >{{ $t("publics.btn.save") }}</el-button
              >
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('CaseSfcsProductCartondelete') !== -1
                "
                @click="removeCase()"
                >{{ $t("publics.btn.delete") }}</el-button
              >
          ...
          <el-tab-pane
            :disabled="PrimaryData == null"
            v-if="$btnList.indexOf('CollectionDataMaintenance') !== -1"
            :label="$t('ProductSetting._29')"
            name="CollectData"
          >
            <div style="margin-bottom: 15px">
              <el-select
                v-model="CollectForm.RESOURCE_ID"
                clearable
                style="width: 150px"
              ...
              <el-button type="primary" @click="CollectSearch">{{
                $t("publics.btn.search")
              }}</el-button>
              <el-button
                type="success"
                v-if="mainTable.STATUS == 0"
                @click.prevent="addCollect(0)"
                >{{ $t("publics.btn.add") }}</el-button
              >
              <el-button
                type="primary"
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('dataSfcsProductResourcesSave') !== -1
                "
                @click="SaveCollect()"
                >{{ $t("publics.btn.save") }}</el-button
              >
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('dataSfcsProductResourcesdelete') !== -1
                "
                @click="removeCollect()"
                >{{ $t("publics.btn.delete") }}</el-button
              >
          ...
          <el-tab-pane
            :disabled="PrimaryData == null"
            v-if="$btnList.indexOf('CollectUIDMaintenance') !== -1"
            :label="$t('ProductSetting._33')"
            name="CollectUID"
          >
            <div style="margin-bottom: 15px">
              <el-select
                v-model="CollectUIDForm.UID_ID"
                clearable
                style="width: 150px"
                :placeholder="$t('ProductSetting._34')"
              >
                <el-option
                  v-for="item in UIDCategory"
                  :key="item.ID"
                  :label="item.OBJECT_NAME"
                  :value="item.ID"
                />
              </el-select>
              <el-button type="primary" @click="CollectUIDSearch">{{
                $t("publics.btn.search")
              }}</el-button>
              ...
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('UIDSfcsProductUidsSave') !== -1
                "
                @click="SaveCollectUID()"
                >{{ $t("publics.btn.save") }}</el-button
              >
              <el-button
                type="danger"
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('UIDSfcsProductUidsdelete') !== -1
                "
                @click="removeCollectUID()"
                >{{ $t("publics.btn.delete") }}</el-button
              >
              <el-button
                type="primary"
          ...
          <el-tab-pane
            :disabled="PrimaryData == null"
            v-if="$btnList.indexOf('CollectionPalletMaintenance') !== -1"
          ...
          >
            <div style="margin-bottom: 15px">
              <el-input
                ...
                style="width: 150px"
              />
              <el-button type="primary" @click.prevent="PalletSearch">{{
                $t("publics.btn.search")
              }}</el-button>
              <el-button
                type="success"
                v-if="mainTable.STATUS == 0"
                @click.prevent="addPallet(0)"
                >{{ $t("publics.btn.add") }}</el-button
              >
              <el-button
                type="primary"
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('PalletSfcsProductPalletSave') !== -1
                "
                @click="SavePallet()"
                >{{ $t("publics.btn.save") }}</el-button
              >
              <el-button
                ...
                v-if="
                  mainTable.STATUS == 0 &&
                  $btnList.indexOf('PalletSfcsProductPalletdelete') !== -1
                "
                @click="removePallet()"
                >{{ $t("publics.btn.delete") }}</el-button
              >
    ...
    >
      <div class="process-header">
        ...
        <el-button
          ...
          icon="el-icon-search"
          @click.native="search_but"
          >{{ $t("SOPRoutes._6") }}</el-button
        >
        <el-button type="primary" @click="process_det_but">{{
          $t("SOPRoutes._31")
        }}</el-button>
    ...
    >
      <el-row :gutter="24" class="dialog-row">
        <el-col :span="12" class="dialog-row-div">
          <img
            :src="img_url + postForm.img"
            :alt="$t('SOPRoutes._60')"
      ...
      <div class="upload-button">
        <el-button type="primary" v-if="ButStatus" @click="edit_save_but">{{
          $t("SOPRoutes._53")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    >
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip">
          最大支持1GB文件大小上传,当文档过大时建议先转换PDT格式
        </div>
      </el-upload>
      ...
      <!-- <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirm_but">{{
          $t("SOPRoutes._31")
        }}</el-button>
      </span> -->

      <span slot="footer" class="dialog-footer">
        <el-button @click="PrimarydialogVisible = false">{{
          $t("SOPRoutes._48")
        }}</el-button>
        <el-button type="primary" @click="PrimarydialogVisible = false">{{
          $t("SOPRoutes._31")
        }}</el-button>
      </span>
    </el-dialog>
    ...
    >
      ...
      <template slot="footer">
        <el-pagination
          :current-page="ProductConFrom.Page"
    ...
    >
      <div style="height: 300px">
        ...
        >
          <vxe-table-column
            ...
            align="center"
            field="FILECODE"
            :edit-render="{ name: 'input', placeholder: '请输入文件编号' }"
          />
          <vxe-table-column
            sortable
            show-overflow
           ...
           <vxe-table-column
            ...
            >
              <template v-slot="{ row }">
                <el-button
                  type="danger"
                  @click="removeFileClick(row, row.$index)"
                  >{{ $t("publics.btn.delete") }}</el-button
                >
              </template>
            </vxe-table-column>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="success" @click="FileSystemAdd"> 新增 </el-button>
        <!-- 取消 -->
        <el-button @click="dialogFileSystem = false">
          {{ $t("_kanban._4") }}
        </el-button>
        <!-- 确定 -->
        <el-button type="primary" @click="FileSystemSubmit">
    ...
    >
      ...
      >
        ...
        <el-form-item>
          <el-select
            v-model="formAdd.BREAK_OPERATION_CODE"
            :placeholder="$t('spmr._9')"
            style="width: 100%"
          >
            <el-option
              v-for="(item, index) in AddSfcsOperations"
              :key="index"
        ...
        <el-form-item>
          ...
          >
            <el-button @click="EvenPlateVisible = false">{{
              $t("spmr._28")
            }}</el-button>
    ...
    >
      <div style="margin-bottom: 10px">
        ...
        <el-button
          ...
          >{{ $t("publics.btn.add") }}</el-button
        >&nbsp;
        <el-button type="primary" icon="el-icon-check" @click="annexSave">{{
          $t("publics.btn.save")
        }}</el-button>
      </div>
      ...
      <div style="margin-top: 10px">
        <el-pagination
          :current-page="ComponentsAattachmentsForm.Page"
    ...
    >
      <div class="process-header">
        <el-select
          placeholder="请选择"
          v-model="workingMoveValue"
          style="width: 100%; margin-top: 20px"
        >
          <el-option
            :value="item.ID"
            :label="item.OPERATION_NAME"
            v-for="(item, index) in this.PrimaryTable"
            :key="index"
            v-show="ProductVal.parentid != item.ID"
          ></el-option>
        </el-select>
      </div>
      <span slot="footer" class="dialog-footer">
...
import { GetRouteOperationByRouteID } from '@/api/SfcsOperations/index.js'
import {
  Index,
...
export default {
  ...
  data () {
    return {
      ...
      validFileRules: {
        FILECODE: [{ required: true, message: '请输入文件编号' }]
      }
    }
  },
  mounted () {
  ...
  methods: {
    FileNumberSystem () {
      this.dialogFileNumberSystem = true
    },
    // 提交文件编号
    FileSystemNumberSubmit () {
      this.$refs.ruleFileNumber.validate(async (valid, errInfo) => {
        if (valid) {
    ...
    // 联动采集零件数据
    // setSameEditComponent (row, type) {
    //   if (type === 'ODM_COMPONENT_PN') {
    //     row.COMPONENT_NAME = this.CollectPartsDataSelect.filter(item => item.COMPONENT_PN === row.ODM_COMPONENT_PN)[0].COMPONENT_NAME
    //     row.COMPONENT_ID = this.CollectPartsDataSelect.filter(item => item.COMPONENT_PN === row.ODM_COMPONENT_PN)[0].COMPONENT_ID
    //   } else {
    //     row.ODM_COMPONENT_PN = this.CollectPartsDataSelect.filter(item => item.COMPONENT_ID === row.COMPONENT_ID)[0].COMPONENT_PN
    //     row.COMPONENT_NAME = this.CollectPartsDataSelect.filter(item => item.COMPONENT_PN === row.ODM_COMPONENT_PN)[0].COMPONENT_NAME
    //   }
    //   row.DATA_FORMAT = this.CollectPartsDataSelect.filter(item => item.COMPONENT_PN === row.ODM_COMPONENT_PN)[0].SPECS
    //   console.log(row)
    // },
    getComponentPN: throttle(function () {
      this.GetSfcsProductComponent()
    }, 2000),
    async GetSfcsProductComponent (createComponent = false) {
      console.log('111111-----Spenan--------11111', this.mainTable)

      // 编辑状态 并且是未审核
      if (this.mainTable.STATUS === 0 && this.mainTable.ID > 0) {
        createComponent = true
      }

      console.log(
        this.mainTable.STATUS === 1 ||
          this.mainTable.PART_NO === '' ||
          this.mainTable.PART_NO === null ||
          this.mainTable.COLUMN_CODE === '' ||
          createComponent === false ||
          this.mainTable.COLUMN_CODE === null
      )
    ...
    makingWork (val) {
      this.mainTable.PART_NO = val
      // this.GetSfcsProductComponent()// Spenan
      this.PartNoClick()
    },
    // 工序-编辑行
    EditClosed ({ column, columnIndex, row }) {
      if (columnIndex === 7 || columnIndex === 8 || columnIndex === 9) {
        if (
          row.STANDARD_HUMAN &&
          row.STANDARD_WORK_FORCE &&
          row.STANDARD_HUMAN !== '' &&
          row.STANDARD_WORK_FORCE !== ''
        ) {
          row.STANDARD_CAPACITY =
            parseInt(3600 / row.STANDARD_WORK_FORCE) * row.STANDARD_HUMAN
        }
      }
    },
    // 提交工序转移
    async workingdialogMove () {
      ...
      const res = await UpdateResourceByParentID({
        ParentId: this.workingMoveValue,
        ResourceId: this.selectDataList
      })
      console.log(res)
      if (res.Result) {
    ...
    //   if (res.Result) {
    //     console.log('getLoadDataByPartNo=')
    //     console.log(res)
    ...
    customClick () {
      ...
      GetRouteOperationByRouteID({ route_id: this.formAdd.route_id }).then(
        (res) => {
          if (res.Result) {
    ...
    async FileSystem () {
      ...
      const res = await LoadSOPFileData({
        Key: this.ProductVal.parentid,
        Page: 1,
        Limit: 999
      })
      console.log(res)
    ...
    // 插入文件系统数据
    FileSystemAdd () {
      const $table = this.$refs.xTableFlie
      $table.insertAt({
        ID: 0,
        TYPE: 0,
        OPERATIONS_ROUTES_ID: this.ProductVal.parentid,
        FILECODE: '',
        FILENAME: '',
        FILEURL: ''
      })
    ...
    removeFileClick (row) {
      ...
      this.$confirm('确定删除该记录吗？', '提示', {
        ...
        if (row.ID <= 0) {
          ...
          const res = await SaveSOPFileData({
            RemoveRecords: [row]
          })
          if (res.Result) {
            $table.remove(row)
    ...
    async getSystem () {
      ...
      if (res.Result) {
        var data = res.Result || []
        if (data.length !== 0) {
          var SysArr = this.SystemList.filter(
            (item) => item.ID === Number(data[0].CONFIG_VALUE)
          )
    ...
    async getLoadAddRoute () {
      ...
      if (data) {
        ...
        const arr1Ids = this.PrimaryTable.map(
          (item) => item.CURRENT_OPERATION_ID
        )
        const result = this.RouteTable.filter(
          (item) => !arr1Ids.includes(item.CURRENT_OPERATION_ID)
        )
        var maxid = 0
    ...
    async getAddOrModify (ID) {
      ...
      if (res.Result) {
        ...
        this.mainTable.COLUMN_CODE = data.COLUMN_CODE // 项目号
    ...
    async getLoadDtlData (val, PART_NO) {
      ...
      if (res.Result) {
        ...
        console.log(this.PrimaryTable[this.ProductVal.PrimaryIndex])
        console.log(this.ProductVal.PrimaryIndex)
        await this.$refs.xTable.setRadioRow(
          this.PrimaryTable[this.ProductVal.PrimaryIndex]
        )
      }
    },
    // 获取 预览编辑
    ...
    primary_cell_click (row, rowIndex, $rowIndex, column) {
      ...
      this.ProductVal.parentid = row.row.ID <= 0 ? -1 : row.row.ID
      this.ProductVal.PrimaryIndex = row.rowIndex
    ...
    process_det_but () {
      ...
      const result = this.procesArr.filter(
        (item) => !arr1Ids.includes(item.Id)
      )
      var maxid = 0
      if (this.PrimaryTable.length > 0) {
        maxid = Math.max.apply(
      ...
      if (this.readonly) {
        ...
        AddOrModifySave(this.mainTable).then((response) => {
          if (response.Result) {
            ...
            this.handleCreated() // Spenan 重新读取
          }
        })
      } else {
        // 新增保存
    ...
    primaryAvatarSuccess (res) {
      ...
      if (res.Result) {
        ...
        this.$message({
          showClose: true,
          message: res.ErrorInfo.Message || '上传失败',
          type: 'warning'
        })
        voice.error()
      }
    ...
    getArrDifSameValue (arr1, arr2, key1, key2) {
      var result = []
    ...
    async GetSfcsProductComponents () {
      this.componentLoading = true
      const res = await GetSfcsProductComponents({
        PART_NO: this.mainTable.PART_NO,
        COLUMN_CODE: this.mainTable.COLUMN_CODE
      }).catch(() => {
      ...
      if (res.Result) {
        ...
        console.log(temp)
        console.log(this.CollectPartsData)
        const $table = this.$refs.Components

        // 对比是否有相同料号 返回相同料号
        let result = this.getArrDifSameValue(
          this.CollectPartsData,
          temp,
          'ODM_COMPONENT_PN',
          'COMPONENT_PN'
        )
        ...
        if (result && result.length <= 0) {
          this.ComponetsTableAddOne()
        } else {
          var postdata = this.$refs.Components.getRecordset()
          if (postdata.insertRecords.length > 0) {
            this.ComponetsTableAddOne()
            return
          }
          result &&
            result.map((item) => {
              var obj = {}
              obj.PART_NO = this.mainTable.PART_NO
              obj.COLLECT_OPERATION_ID = this.PrimaryData.CURRENT_OPERATION_ID
              obj.COMPONENT_ID = this.ComponentName[0].ID // 零件名称
              obj.ODM_COMPONENT_PN = item.COMPONENT_PN // 零件料号
              obj.DATA_FORMAT = '' // 格式设定
              obj.COMPONENT_QTY = 1 // 数量
              obj.SERIALIZED = 'N' // 是否唯一序列
              obj.ATTRIBUTE2 = item.SPECS // 零件描述
              obj.CUSTOMER_COMPONENT_PN = '' // 客户料号
              obj.ENABLED = 'Y' // 是否激活
              const { row: newRow } = $table.insertAt(obj, -1)
              $table.setActiveCell(newRow)
            })
        }
      }
    },
    ComponetsTableAddOne () {
      const $table = this.$refs.Components
      const record = {
        PART_NO: this.mainTable.PART_NO,
        COLLECT_OPERATION_ID: this.PrimaryData.CURRENT_OPERATION_ID,
        COMPONENT_ID: this.ComponentName[0].ID, // 零件名称
        ODM_COMPONENT_PN: '', // 零件料号
        DATA_FORMAT: '', // 格式设定
        COMPONENT_QTY: 1, // 数量
        SERIALIZED: 'N', // 是否唯一序列
        ATTRIBUTE2: '', // 零件描述
        CUSTOMER_COMPONENT_PN: '', // 客户料号
        ENABLED: 'Y' // 是否激活
      }
      ...
      if (
        ...
        this.$refs.Components.validate((valid) => {
          if (valid) {
            ...
            let tempList = []
            // 有添加的情况
            if (this.CollectPartsSave.InsertComponents.length > 0) {
              tempList = tempList.concat(
                this.CollectPartsSave.InsertComponents
              )
            }
            // 表格有值情况下
            if (this.CollectPartsData && this.CollectPartsData.length > 0) {
              tempList = tempList.concat(this.CollectPartsData)
            }
            console.log(tempList)
            for (var i = 0; i < tempList.length - 1; i++) {
              for (var j = i + 1; j < tempList.length; j++) {
                if (
                  tempList[i].ODM_COMPONENT_PN === tempList[j].ODM_COMPONENT_PN
                ) {
                  return this.$message({
                    message:
                      '零件料号第「' +
            ...
            console.log('提交的值' + this.CollectPartsSave)
            ComSaveData(this.CollectPartsSave).then((res) => {
              ...
              if (res.Result) {
                ...
                this.getLoadDtlData(this.mainTable.ID, this.mainTable.PART_NO)
              }
            })
          }
        })
    ...
    async removeCollectPart () {
      ...
      this.$confirm(this.$t('ssn._15'), this.$t('ssn._16'), {
        ...
        .then(async () => {
          let record = this.$refs.Components.getRadioRecord()
...
.SOPUploadFile .el-upload {
  width: 100%;
}
.SOPUploadFile .el-upload-dragger {
  width: 100%;
}
.SOPUploadFile
  v-deep.el-upload-list__item.is-success.focusing
  .el-icon-close-tip {
