<template>
  <div>
    <div style="text-align:center;box-shadow: 0 0 4px #e5e5e5;height: 44px;background: #fff;position: fixed;top: 0;width: 100%">
      ...
      <h1 style="font-size: 18px;line-height: 44px;color: #333;margin: 0;"><i class="el-icon-tickets" style="margin-right:5px"></i>{{$t('homePage._45')}}</h1>
...
<!--            <el-tag v-if="item.TAG" type="primary" size="mini" style="margin-right: 5px">{{ item.TAG }}</el-tag>-->
          ...
          <div class="right">{{item.UPDATED_BY}} {{$t('homePage._40')}} {{ item.UPDATED_TIME }}</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getPageList () {
      this.loading = true
      const res = await MVCLoadData(this.formData)
      this.loading = false
