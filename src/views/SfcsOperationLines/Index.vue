<template>
  <d2-container>
    <template slot="header">
      ...
      >
        <el-input
          v-model="formData.OPERATION_LINE_NAME"
        ...
        <el-button
          ...
          v-if="$btnList.indexOf('SfcsOperationLinesAdd') !== -1"
        ...
        >
          <template slot-scope="scope">
            <div>
              <el-switch
                :disabled="
                  $btnList.indexOf('SfcsOperationLinesChangeEnabled') === -1
                "
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              v-if="$btnList.indexOf('SfcsOperationLinesEdit') !== -1"
    ...
    >
      ...
      >
        ...
        <el-form-item :label="$t('SfcsOperationLines._9')" prop="LINE">
          <el-input
            v-model="form.LINE"
            @change="LineBlur"
        ...
        <el-form-item
          :label="$t('SfcsOperationLines._11')"
          type="text"
          prop="OPERATION_LINE_NAME"
        >
          <el-input
            v-model="form.OPERATION_LINE_NAME"
            :placeholder="$t('SfcsOperationLines._12')"
          />
        </el-form-item>
        <el-form-item :label="$t('SfcsOperationLines._13')" prop="PLANT_CODE">
          <el-select
        ...
        <div class="call-dialog-button">
          <el-button type="success" icon="el-icon-check" @click="shout_but"
            >&nbsp;{{ $t("publics.btn.makeSure") }}&nbsp;</el-button
          >
          <el-button type="danger" icon="el-icon-close" @click="close_btn"
      ...
      if (!this.form.OPERATION_LINE_NAME || this.form.ID === 0) {
        if (this.LINEName) {
          this.form.OPERATION_LINE_NAME =
            this.PhysicalName + '的' + this.LINEName
        }
      }
    },
    LineBlur (row) {
      ...
      if (
        (!this.form.OPERATION_LINE_NAME || this.form.ID === 0) &&
        this.LINEName
      ) {
        ...
        this.$refs.callVal.validate(async (valid, errInfo) => {
          if (valid) {
            if (
              ...
              this.form.ORGANIZE_ID = this.form.ORGANIZE_ID[
                this.form.ORGANIZE_ID.length - 1
              ]
            }
